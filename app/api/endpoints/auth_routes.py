...
async def register(user_in: UserCreate, db: Session = Depends(get_db)):
    ...
    if user_exists:
        raise HTTPException(
            status_code=400, detail="Este e-mail já está cadastrado no PrecedentIA."
        )

    new_user = User(email=user_in.email, password=hash_password(user_in.password))
    ...
    return {
        "message": "Usuário criado com sucesso! Faça login para validar seu acesso."
    }
...
async def login(user_in: UserLogin, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao enviar e-mail de autenticação: {str(e)}"
        )
